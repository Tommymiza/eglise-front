...
export default function SacramentChart() {
  ...
  const generateYearList = () => {
    ...
    const yearSet = new Set(
      sacramentList
        .flatMap((s) => s.christians)
        .map((item) => new Date(item?.createdAt).getFullYear().toString())
    );
    const yearList: string[] = [...yearSet];
    setYears(yearList);
  };
  useEffect(() => {
    generateYearList();
  ...
  useEffect(() => {
    const newData = sacramentList.map((sacrament) => ({
      name: sacrament.name,
      length: sacrament.christians
        ?.filter((c) =>
          new Date(c.createdAt).getFullYear().toString().includes(year)
        )
        ?.filter((c) => c.church_id.includes(church)).length,
    }));
    setData(newData);
